...
public class NetworkUtils {
    ...
    public static final int NETWORK_NO_AIR = 0;         //飞行模式
    ...
    public static int getNetWorkType(Context context) {
        ...
        if (isAirplaneModeOn(context)){ //是否是飞行模式
            netType = NETWORK_NO_AIR;
        }else {
            NetworkInfo info = getActiveNetworkInfo(context);
            if (info != null && info.isAvailable()) {

                if (info.getType() == ConnectivityManager.TYPE_WIFI) {
                    if (isWifiConnected(context) && connectionNetwork()){//判断是wifi同时也能连接网络
    ...
    public static int getNetWorkTypeName(Context context) {
        switch (getNetWorkType(context)) {
            case NETWORK_NO_AIR: //飞行模式 airplane
                return 0;
            case NETWORK_WIFI:
                return 1;
            case NETWORK_4G:
    ...
    public static boolean checkNetworkState(Context context) {
        boolean flag = false;
        int stateNum = getNetWorkTypeName(context);
        switch (stateNum){
            case 0:
                flag = false;
                Toast.makeText(context, "请关闭飞行模式后重试", Toast.LENGTH_SHORT).show();
                break;
            case 1:
                flag = true;
                break;
